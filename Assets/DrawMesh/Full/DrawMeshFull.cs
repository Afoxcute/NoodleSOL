...
public class DrawMeshFull : MonoBehaviour
{
    ...
    private void Update()
    {
        if (!UtilsClass.IsPointerOverUI())
        {
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                lastGameObject.GetComponent<MeshCollider>().sharedMesh = mesh;
                lastGameObject.GetComponent<MeshCollider>().convex = true;
                lastGameObject.GetComponent<MeshCollider>().isTrigger = true;
                lastGameObject.GetComponent<Rigidbody>().useGravity = false;
                Material material = new(drawMeshMaterial)
                {
                    color = lineColor
                };
    ...
    {
        lastGameObject = new GameObject("DrawMeshSingle", typeof(MeshFilter), typeof(MeshRenderer), typeof(MeshCollider), typeof(Rigidbody));
        lastSortingOrder++;
